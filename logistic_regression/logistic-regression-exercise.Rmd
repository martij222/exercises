...
```{r}
# Load NHIS data
NH11 <- as.tibble(readRDS("logistic_regression/dataSets/NatHealth2011.rds"))
NH11
...
ex2$everwrk <- factor(ex2$everwrk, levels = c("2 No", "1 Yes"))

# Check each column for missing values
summary(ex2)
```

Given that over half of the target variable is missing data, we'll have to impute these values before building the regression model. To this end, we will use the `mice` (Multiple Imputation by Chained Equations) package.

```{r}
# Load the package
library(mice)

# Set seed and impute values
set.seed(42)
ex2.imputed <- complete(mice(ex2))

# Check for NA's
summary(ex2.imputed)

# Build model
model <- glm(everwrk ~ age_p + r_maritl, data = ex2.imputed, family = "binomial")
summary(model)

# What is the probability of working for each level of marital status (for the mean age surveyed)?

# Manually
pred <- with(ex2.imputed,
                expand.grid(r_maritl = levels(NH11$r_maritl)[-c(1, 4)],
                            age_p = mean(age_p)))

cbind(pred, predict(model, type = "response",
                    se.fit = TRUE, interval = "confidence",
                    newdata = pred))
